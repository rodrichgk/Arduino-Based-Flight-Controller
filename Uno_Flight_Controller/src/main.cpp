...
void stopAll();
bool isStarted(int switchValue);
void readSensor();
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    pulse_length[mode_mapping[THROTTLE]] = data.substring(10, pos1).toInt();
    pulse_length[mode_mapping[YAW]]  = data.substring(pos1 + 6, pos2).toInt();
    pulse_length[mode_mapping[ROLL]]  = data.substring(pos2 + 7, pos3).toInt();
    pulse_length[mode_mapping[PITCH]]  = data.substring(pos3 + 8, pos4).toInt();
    switch_state = data.substring(pos4 + 9).toInt();
  }
...
bool isStarted(int switchValue) {
    if (switchValue == 2000 && status == STOPPED && pulse_length[mode_mapping[THROTTLE]] <= 1012) {
        status = STARTED;
        resetPidController();
        resetGyroAngles();
    }
    else if (switchValue == 1000 && status == STARTED) {
        status = STOPPED;
        stopAll();
    }
